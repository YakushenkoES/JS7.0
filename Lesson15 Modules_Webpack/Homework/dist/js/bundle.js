...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _Animation__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./Animation */ "./src/js/parts/Animation.js");
// Calculator ___________________________


function calculator(elCalculator) {
  var anim = new _Animation__WEBPACK_IMPORTED_MODULE_0__["default"]();

  var prepareValue = function prepareValue(val) {
    return val.replace(/^0+|[^0-9]+/g, "");
  }; // Убрать все не цифры и нули в начале} 


  var getPlaceCoeff = function getPlaceCoeff() {
    return place.options[place.selectedIndex].value;
  };

  var calculatorInputs = elCalculator.querySelectorAll('.counter-block-input'),
...
function forms(forms) {
  ...
  forms.forEach(function (form) {
    var statusMessage = createReqStatusEl(form); // Создать элемент статуса запроса
      ...
      function waitDone(_xhr) {
        return new _Promise(function (resolve, reject) {
          ...
          _xhr.addEventListener('readystatechange', function onWaitDone() {
            if (_xhr.readyState === 4) {
              // Done ждать ТОЛЬКО readyState 4
              if (_xhr.status === 200) {
                // Done OK
...
function maskInputPhone(inputs) {
  var maskPhone = "+7 (___) ___ __ __"; // Маска ввода
  //document.querySelectorAll('input[type=tel]')

  inputs.forEach(function (tel) {
...
function popup() {
  var moreBtns = document.querySelectorAll(".more, .description-btn"),
...
function slider(elSlider) {
  ...
  var slideIndex = 1,
      ...
      anim = new _Animation__WEBPACK_IMPORTED_MODULE_0__["default"]();
  ...
  function showSlide(n) {
    ...
    function drawSlider(progress) {
      slides[n - 1].style.filter = "sepia(".concat((1 - progress) * 100, "%)");
      var rt = -10 * (1 - progress);
      slides[n - 1].style.transform = "rotateY(".concat(rt, "deg) rotateX(").concat(rt, "deg)");
    }
  ...
  dotsWrap.addEventListener('click', function (e) {
    if (e.target && e.target.matches('.dot')) {
      var ind = Array.from(dots).indexOf(e.target);
...
function tabs(elTab) {
  ...
  var tab = elTab.querySelectorAll(".info-header-tab"),
